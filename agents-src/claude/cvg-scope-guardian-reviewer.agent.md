---
name: cvg-scope-guardian-reviewer
...
- `Document type:` — the orchestrator's authoritative classification (`requirements` or `cvg-plan`). Trust it; do not re-classify.
...
- Missing requirements (coherence reviewer), security (cvg-security-lens-reviewer)
- Design/UX (design-lens), technical feasibility (cvg-feasibility-reviewer)
