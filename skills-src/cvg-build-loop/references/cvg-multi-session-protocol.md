...
This file is a skill-local copy of the `cvg-multi-session` protocol needed by `cvg-build-loop`. It keeps the loop skill self-contained when installed from a marketplace cache.

Use this protocol when the current session should act as an **orchestrator** over other real specialist sessions. This is a protocol, not just a checklist.
