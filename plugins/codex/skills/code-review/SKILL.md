...
- `ce-correctness-reviewer` - "Review for logic errors, edge cases, state bugs,
  error propagation. Intent: <intent>. Plan: <path>. Base: <base ref>."
- `ce-testing-reviewer` - "Review for coverage gaps, weak assertions, brittle
...
- `ce-security-reviewer` - "Review for security vulnerabilities. Intent:
...
- `ce-adversarial-reviewer` - "Review for failure scenarios and abuse cases.
  Intent: <intent>. Plan: <path>. Base: <base ref>."

**When diff touches error handling, retries, timeouts, or background jobs:**

- `ce-reliability-reviewer` - "Review for production reliability issues.
  Intent: <intent>. Plan: <path>. Base: <base ref>."
