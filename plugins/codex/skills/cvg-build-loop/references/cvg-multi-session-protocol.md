...
This file is a skill-local copy of the `cvg-multi-session` protocol needed by `cvg-build-loop`. It keeps the loop skill self-contained when installed from a marketplace cache. Generated by `bun run sync` from `skills-src/cvg-multi-session/SKILL.md`; do not edit by hand.
