...
description: Orchestrate monitored multi-agent workflows with specialist handoff, callback transport, role-specific review feedback handling, and fresh-reviewer exit gates. Runs on Codex threads or Claude Code background agents. Use when the user asks to coordinate multiple agent sessions, run an orchestrator workflow, supervise planning or implementation, or manage worker/reviewer loops.
argument-hint: "[goal, plan path, worktree, or workflow description]"
---

# Multi-Session Workflow

Use this skill when the current session should act as an **orchestrator** over other real specialist sessions. This is a protocol, not just a checklist.
...
tool — the thread id from `create_thread` on Codex, the agent id from the
`Agent` tool result on Claude Code — never an id the specialist writes in prose.
Do not accept placeholder specialist identity in callback templates.
...
If neither transport's tools are available after searching for them, report a
...
- Verify every specialist id immediately after creation or selection: on Codex, `read_thread` the returned id; on Claude Code, the `Agent` tool result already carries the authoritative agent id and needs no separate read.
- (Codex) For a newly created specialist, send a short identity note after `read_thread`
  succeeds: `Your verified thread id is <specialist-thread-id>. Use this
  exact id in callbacks.` On Claude Code, skip this; the harness tracks identity.
...
- If the platform specialist tools are not loaded, search for them first (`create_thread`, `send_message_to_thread`, `read_thread` on Codex; `Agent`, `SendMessage`, `TaskOutput` on Claude Code). If they still cannot be loaded, report a tool-layer blocker.

### Gate 2: Orchestrator Callback Transport

Every specialist prompt must include:

- the callback destination: on Codex, the destination orchestrator thread id
  plus the instruction to send the callback with `send_message_to_thread`; on
  Claude Code, the instruction that the specialist's final message is the
  callback and must match the callback template,
...
On Codex, the specialist must send its callback to the orchestrator thread; a
final answer left only in the specialist thread is not sufficient. On Claude
Code, the harness delivers the specialist's final message to the orchestrator
as the completion notification; the specialist must format that final message
as the callback.
...
The orchestrator treats the phase as pending until the callback is visible: on
Codex, in the orchestrator thread or manually relayed by the user; on Claude
Code, as the specialist's completion notification.
...
3. (Codex) Send the verified thread id to newly created specialists, then
   create or update a heartbeat automation for the current orchestrator thread.
4. Tell the user the specialist id (and the heartbeat id on Codex).
5. End the active turn. On Claude Code, the harness re-invokes the orchestrator
   with a `<task-notification>` when the specialist completes; no heartbeat
   exists or is needed.
...
- specialist ids verified per Gate 1 (thread ids on Codex, agent ids on Claude Code),
