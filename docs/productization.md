...
Marketplace installs use versioned cache directories. A skill cannot rely on sibling skill paths such as `../cvg-multi-session/SKILL.md`. When loop skills need the cvg-multi-session protocol, they carry a skill-local copy under `references/cvg-multi-session-protocol.md`. These copies are generated by `bun run sync` from `skills-src/cvg-multi-session/SKILL.md` (the single source of truth) and exist only in the platform roots; do not edit them by hand.

### Platform-specific skill content

`skills-src/` markdown may wrap Codex-only or Claude-Code-only blocks in standalone `<!-- codex -->`/`<!-- claude -->` marker lines. `bun run sync` keeps only the matching platform's blocks in each generated root, so a Claude Code install carries no Codex heartbeat mechanics and vice versa. The generic root ships platform-neutral base skills and must stay marker-free.
